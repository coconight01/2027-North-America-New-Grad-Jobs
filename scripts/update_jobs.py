...
TIMEOUT = 25
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; 2027-New-Grad-Jobs/0.2; +https://github.com/coconight01/2027-North-America-New-Grad-Jobs)"}
...
CITIZEN = re.compile(r"\b(?:u\.?s\.?|united states) citizenship (?:is )?required|must be (?:a )?u\.?s\.? citizen|active (?:security )?clearance|ability to obtain (?:a )?(?:security )?clearance\b", re.I)
...
def eligible(j,include_general):
    text=" ".join([j["role"],j.get("description",""),j.get("graduation",""),j.get("start_date","")])
    if EXCLUDE.search(j["role"]): return False
    if OLD_YEAR.search(text) and not YEAR_2027.search(text): return False
    if not NORTH_AMERICA.search(j["location"]+" "+j.get("country","")): return False
...
def github_readme(c):
    ref=c.get("ref","main"); r=requests.get(f"https://raw.githubusercontent.com/{c['repo']}/{ref}/README.md",headers=HEADERS,timeout=TIMEOUT); r.raise_for_status(); text=r.text; out=[]
    ...
    for row in soup.find_all("tr"):
        ...
        if company and role and links: out.append(job(company,role,loc,links[0],f"GitHub:{c['repo']}"))
    ...
    for line in text.splitlines():
        ...
        company=re.sub(r"[*_`]", "", cells[0]); role=re.sub(r"[*_`]", "", cells[1]); loc=cells[2]; url=next((u for _,u in reversed(links) if "github.com/" not in u),links[-1][1])
        if company and role: out.append(job(company,role,loc,url,f"GitHub:{c['repo']}"))
    return out
...
def enrich(j):
    ...
    if CITIZEN.search(text): j["citizenship_required"]="Yes"
    ...
    found=[name for name,p in SKILLS if p.search(text)]; j["skills"]=", ".join(found[:10]); j["description"]=text[:80000]; j["last_verified"]=TODAY
    return j

def merge(a,b):
    for k,v in b.items():
        if k=="description" and len(v)>len(a.get(k,"")): a[k]=v
        elif (not a.get(k) or a.get(k) in {"Unknown","Not listed","Other"}) and v not in {"","Unknown","Not listed","Other"}: a[k]=v
    if b.get("last_verified","")>a.get("last_verified",""): a["last_verified"]=b["last_verified"]
    return a
def dedupe(rows):
    out={}
    for j in rows:
        j["url"]=canon(j.get("url","")); j["category"]=j.get("category") if j.get("category") not in {"","Other"} else classify(j["role"]+" "+j.get("description",""))
        key=j["url"] or "|".join(re.sub(r"\W+","",j.get(x,"").lower()) for x in ["company","role","location"])
        out[key]=merge(out[key],j) if key in out else j
    return sorted(out.values(),key=lambda x:(x.get("status")!="Open",x.get("date_added",""),x.get("company","").lower()))
def load_csv(path):
...
def write(rows):
    for j in rows:
        for f in FIELDS: j.setdefault(f,"" if f not in {"salary"} else "Not listed")
    with (ROOT/"data/jobs.csv").open("w",encoding="utf-8",newline="") as f: w=csv.DictWriter(f,fieldnames=FIELDS,extrasaction="ignore"); w.writeheader(); w.writerows(rows)
    (ROOT/"data/jobs.json").write_text(json.dumps([{k:j.get(k,"") for k in FIELDS} for j in rows],ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    open_rows=[j for j in rows if j.get("status")=="Open"]
    groups={}
    for j in open_rows: groups.setdefault(j.get("category","Other"),[]).append(j)
    parts=["# 2027 North America New Grad Full-Time Jobs","",f"> Last automated update: **{TODAY}** · Open roles: **{len(open_rows)}**","","Automatically discovers roles from public ATS feeds and GitHub job repositories, then visits eligible job pages to extract visa language, salary ranges, and common skills.","","## Legend","","- ✅ explicit sponsorship support; ❌ explicit no-sponsorship language; ❔ not clearly stated.","- Skill and compensation fields are best-effort extractions; always verify on the employer page.",""]
    for cat in ["Software Engineering","AI / Machine Learning","Data Engineering","Infrastructure / Systems","Quantitative Finance","Hardware Engineering","Cybersecurity","Product Management","Finance / Research","Other"]:
        ...
        for j in sorted(groups[cat],key=lambda x:(x.get("date_added",""),x.get("company","")),reverse=True):
            visa="✅" if j.get("sponsorship")=="Yes" else ("❌" if j.get("sponsorship")=="No" else "❔"); role=f"[{j['role'].replace('|','/')} ]({j['url']})" if j.get("url") else j["role"]; skills=(j.get("skills") or "—").replace("|","/")
            parts.append(f"| **{j['company'].replace('|','/')}** | {role} | {j['location'].replace('|','/')} | {visa} | {(j.get('salary') or 'Not listed').replace('|','/')} | {skills} | {j.get('date_added','')} |")
        parts.append("")
...
def main():
    ...
    if not a.skip_enrichment:
        ...
        rows=dedupe(rows)
    if a.dry_run: print(f"{len(rows)} eligible jobs"); [print(x['company'],'-',x['role'],'-',x['location']) for x in rows[:30]]; return
